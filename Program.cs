...
class Program
{
    ...
    private static string _svcurl = "https://services.odata.org/TripPinRESTierService/(S(jcidpq3drpzq0hefuabvtj5f))/";
    static async Task<int> Main(string[] args)
    {

        var cl = new ODataClient( _svcurl );

        var model = (EdmModelBase) await cl.GetMetadataAsync();
        ...
        foreach ( var entitySet in entitySets )
        {
            ...
            foreach ( var structProp in type.DeclaredStructuralProperties() )
            {
                ...
                {
                    var def = (IEdmNamedElement) structProp.Type.Definition;
                    Console.WriteLine("---Prop: " + structProp.Name + " / " + def.Name );
                }
                else
                {
                    var def = (IEdmCollectionType) structProp.Type.Definition;
                    var elemType = (IEdmNamedElement) def.ElementType.Definition; 
